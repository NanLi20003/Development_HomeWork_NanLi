...
sSeason = [0.085 0.171 0.043
 ...
 kappa = (theta*yc)*(1/(hm^2));
  ...
  lneYMatrix = zeros(nHousehold,nYear,nMonth);
  for h = 1:nHousehold
      for y = 1:nYear
          eYMatrix(h,y,:)=lognrnd(0, sqrt(sigmaE))*exp(-sigmaE/2);
          lneYMatrix(h,y,:)=lognrnd(0, sqrt(sigmaE)).^(-1)*exp(-sigmaE/2);
      end    
  end    
...
lpeYMatrix = eYMatrix; % if it is highly correlated with consumption
%lneYMatrix negative correlated  
...
lplsSeasonMatrix = lsSeasonMatrix;
 ...
 %% calculate labor
 nonSeasonL = lzMatrix.*lpeYMatrix*hm;
 mmSeasonL = lzMatrix.*lpeYMatrix.*lpmSeasonMatrix.*lpmsSeasonMatrix*hm;
 mhSeasonL = lzMatrix.*lpeYMatrix.*lpmSeasonMatrix.*lphsSeasonMatrix*hm;
 mlSeasonL = lzMatrix.*lpeYMatrix.*lpmSeasonMatrix.*lplsSeasonMatrix*hm;
 hmSeasonL = lzMatrix.*lpeYMatrix.*lphSeasonMatrix.*lpmsSeasonMatrix*hm;
 hhSeasonL = lzMatrix.*lpeYMatrix.*lphSeasonMatrix.*lphsSeasonMatrix*hm;
 hlSeasonL = lzMatrix.*lpeYMatrix.*lphSeasonMatrix.*lplsSeasonMatrix*hm;
 lmSeasonL = lzMatrix.*lpeYMatrix.*lplSeasonMatrix.*lpmsSeasonMatrix*hm;
 lhSeasonL = lzMatrix.*lpeYMatrix.*lplSeasonMatrix.*lphsSeasonMatrix*hm;
 llSeasonL = lzMatrix.*lpeYMatrix.*lplSeasonMatrix.*lplsSeasonMatrix*hm;
...
% determistic part
lnhSeasonMatrix = hSeasonMatrix;
lnmSeasonMatrix = mSeasonMatrix;
lnlSeasonMatrix = lSeasonMatrix;
...
% stochastic part
 ...
 %% calculate labor
 mmSeasonLN = lzMatrix.*lneYMatrix.*lnmSeasonMatrix.*lnmsSeasonMatrix*hm;
 mhSeasonLN = lzMatrix.*lneYMatrix.*lnmSeasonMatrix.*lnhsSeasonMatrix*hm;
 mlSeasonLN = lzMatrix.*lneYMatrix.*lnmSeasonMatrix.*lnlsSeasonMatrix*hm;
 hmSeasonLN = lzMatrix.*lneYMatrix.*lnhSeasonMatrix.*lnmsSeasonMatrix*hm;
 hhSeasonLN = lzMatrix.*lneYMatrix.*lnhSeasonMatrix.*lnhsSeasonMatrix*hm;
 hlSeasonLN = lzMatrix.*lneYMatrix.*lnhSeasonMatrix.*lnlsSeasonMatrix*hm;
 lmSeasonLN = lzMatrix.*lneYMatrix.*lnlSeasonMatrix.*lnmsSeasonMatrix*hm;
 lhSeasonLN = lzMatrix.*lneYMatrix.*lnlSeasonMatrix.*lnhsSeasonMatrix*hm;
 llSeasonLN = lzMatrix.*lneYMatrix.*lnlSeasonMatrix.*lnlsSeasonMatrix*hm;
